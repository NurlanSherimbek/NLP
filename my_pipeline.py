...
if __name__ == '__main__':
    ...
    labels=df['measure']
    labels1=df['unit_of_measurement']
    
    
    param_grid = {'vectorizer__method': ['tfidf', 'bw'],
                  'vectorizer__min_df': [0.001, 0.01, 0.02,0.05],
                  'vectorizer__max_df': [0.9, 0.95, 0.98,1.],
                  'vectorizer__ngram_range': [(1,1), (1,2), (1,3)],
                  'model__name_model': ['LR', 'LinearSVC',  'SVC', 'KNC', 'RFC', 'GBC']
                  
                    }
    
    rez=test_model_linear(df, labels, param_grid=param_grid, n_iter=10)
    try:
