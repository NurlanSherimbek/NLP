...
if __name__ == '__main__':
    ...
    vect=vectorizer(method='bw', min_df=0.01, max_df=0.95, ngram_range=(1,2))
    Result=df['clr_text']
    vect=vect.fit(Result)
    df1=vect.transform(Result)
    df1['out_measure']=df['measure']
    df1['out_unit_of_measurement']=df['unit_of_measurement']
    df1.to_excel('Data_vect.xlsx')
