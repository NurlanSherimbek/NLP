...
class my_model(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, labels = None, ):
        ...
        if self.name_model=='LR':
            ...
            lr=MLPClassifier(max_iter=1000)
...
if __name__ == '__main__': 
    ...
    labels=df['out_measure']
    labels1=df['out_unit_of_measurement']
    del df['out_measure']
    del df['out_unit_of_measurement']
    model=my_model(name_model='LR')
    ...
    model=my_model(name_model='KNC')
    X_train, X_test, y_train, y_test = train_test_split(df, labels1, test_size=0.3, stratify=labels1, random_state=42)
    model.fit(X_train, y_train)
    score=model.score(X_test, y_test)
    print(score)
